...
accelerometer data, and additionally that you have cleaned and stored
your additional sensor data as RData files.

As any sensor implies different data cleaning necessities, this is
...
contain a two-column data frame, including: time stamp in ISO8601 format and 
the output of interest.

Also, it is important that the RData files do only contain the data
frame, with no additional objects. For example, the sample raw csv data
...
This file was read and cleaned so that the RData output looks like this:

```{r cleanfile, echo=TRUE}
# time = cleanCGM::formatTime(raw, ts_indices = 1:nrow(raw))
knitr::kable(head(cgm_cleaned))
```
